...
/**
 * Provides access to the underlying builders for {@link SetBuilder}, {@link ListBuilder}, {@link
 * DataBuilder}, and {@link MapBuilder}.
 *
 * @param <T> The {@link java.lang.reflect.Type} of the return data
 */
public interface PojoBuilder<T> {

  /** Return a {@link java.util.Set} of fake pojo. */
  SetBuilder<T> toSet();

  /** Return a {@link java.util.List} of fake pojo. */
  ListBuilder<T> toList();

  /**
   * Use a list of maps to apply overrides to each iteration of the generated data. <br>
   * <br>
   * Useful for generating a list of Managers first, later using that same list to generate a list
   * of Employees reporting to them, etc.
   *
   * @param valueMapList A list of maps used to override any number of attributes per Pojo per
   *     iteration
   */
